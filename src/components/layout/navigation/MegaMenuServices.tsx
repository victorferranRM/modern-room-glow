...
import { serviceHrefs } from "./navigation-data";
...
type SectionKey = "operations" | "emergencies" | "integration";

const columnConfig: { key: SectionKey; icons: typeof operationsIcons }[] = [
  { key: "operations", icons: operationsIcons },
  { key: "emergencies", icons: emergencyIcons },
  { key: "integration", icons: integrationIcons },
];
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-8">
      <div className="grid grid-cols-3 gap-12">
        {columnConfig.map(({ key, icons }) => {
          const section = tObject<{
            title: string;
            items: { title: string; description: string; note?: string }[];
          }>(`megaMenu.services.${key}`);
          const hrefs = serviceHrefs[key];

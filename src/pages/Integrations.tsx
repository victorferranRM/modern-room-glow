...
// Logo grid for hero section
const heroLogos = integrations.slice(0, 12).map(i => ({ name: i.name, initial: i.name.charAt(0) }));
...
export default function Integrations() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero Section - Lodgify style with logo grid */}
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <AnimatedSection animation="fade-up" className="space-y-8">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground leading-tight">
                Connect Roomonitor with your{" "}
                <span className="text-primary">favorite tools</span>
              </h1>
              
              <p className="text-lg text-muted-foreground max-w-xl">
                Roomonitor integrates with the industry's top tools and apps to ensure you have what you need to manage all aspects of your vacation rental operations:
              </p>
            ...
            {/* Right - Logo Grid with scroll animation */}
            <div ref={logoGridRef} className="relative">
              <div className="grid grid-cols-4 gap-4">
                {heroLogos.map((logo, index) => (
                  <div
                    key={logo.name}
                    className={`bg-background rounded-2xl shadow-sm border border-border/50 p-6 flex items-center justify-center aspect-square transition-all duration-700 hover:shadow-lg hover:border-primary/30 hover:scale-105 ${
                      isVisible
